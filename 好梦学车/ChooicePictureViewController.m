//
//  ChooicePictureViewController.m
//  好梦学车
//
...
#define kWHC_CellName             (@"ChooicePictureViewController")

#define kChoiceTitle              (@"已选择(%d / %d)")

@interface ChooicePictureViewController ()<WHC_PhotoListCellDelegate,UITableViewDelegate,UITableViewDataSource>{
    NSMutableArray         * _assetArr;          //相册集合
    NSInteger                _listColumn;        //列表列
    BOOL                     _choiceState;       //选择的状态
    
    
    __weak  typeof(ChooicePictureViewController)     * _sf;
}
@property (weak, nonatomic) IBOutlet UITableView *pictureTV;

@end

@implementation ChooicePictureViewController
...
-(void)selectedAssets:(NSArray*)assets{
    ...
    dispatch_sync(dispatch_get_main_queue(), ^{
        [self dismissViewControllerAnimated:YES completion:^{
            if(_delegate){
                if([_delegate respondsToSelector:@selector(WHCChoicePictureVC:didSelectedPhoto:)]){
                    [_delegate WHCChoicePictureVC:_sf didSelectedPhoto:imageArr[0]];
                }
            }
        }];
    });
