...
@interface ExameNewsTableViewCell : UITableViewCell
...
@implementation ExameNewsTableViewCell
...
@interface ExameNewsViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ExameNewsViewController
...
- (void)getData{
    ...
    if (userId && userId.length >0) {
        dic =@{@"msgType":@"EXAM",@"platform":@"",@"pushClient":@"STUDENT",@"pushStatus":@"",@"tags":userId};//0->未开始学习，1->学习中，2->暂停学习，3->申请考试，4->考试通过，5->补考中, 6->考爆
    }else{
        dic =@{@"msgType":@"EXAM",@"platform":@"",@"pushClient":@"STUDENT",@"pushStatus":@"",@"tags":@""};//0->未开始学习，1->学习中，2->暂停学习，3->申请考试，4->考试通过，5->补考中, 6->考爆
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ExameNewsTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:index];
    if (!cell) {
        cell = [[ExameNewsTableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:index];
    }
...
- (void)commitEvaluateActiveWithCoachId:(NSString *)recordId andDes:(NSString *)des andPint:(NSString *)point{
    ...
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error == nil) {
            ...
            __block NSString *messageStr = [jsonDict objectForKey:@"message"];
            NSString *success = [NSString stringWithFormat:@"%@",[jsonDict objectForKey:@"success"]];
            dispatch_async(dispatch_get_main_queue(), ^{
...
//            [self refreshNewStateWithMdel:model];
            if (success.boolValue) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    //验证码输入错误
                    UIAlertController *v = [UIAlertController alertControllerWithTitle:@"错误提示" message:messageStr preferredStyle:UIAlertControllerStyleAlert];
                    UIAlertAction *active = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
