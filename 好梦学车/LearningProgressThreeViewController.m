...
- (void)setTitle2Str:(NSString *)title2Str{
    if ([title2Str isEqualToString:@"注:考试前请缴考试费！"]) {
...
- (void)setTitle4Str:(NSString *)title4Str{
    if ([title4Str isEqualToString:@"注:考试前请缴考试费！"]) {
    ...
    backGroundView.frame = CGRectMake(20, 5, CURRENT_BOUNDS.width-40, 170);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        cell.title4Str = @"注:考试前请缴考试费！";
        cell.title1Font = 14;
    }else if (indexPath.section == 2){
        cell.title1Str = @"科三考试合格后1小时进行安全文明考试";
        cell.title2Str = @"注:考试前请缴考试费！";
