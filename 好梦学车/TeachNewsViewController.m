...
//    NSRange range2 = {0,mutStr.length};
    ...
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error == nil) {
            ...
            __block NSString *errorStr = [jsonDict objectForKey:@"message"];
            //此处为测试数据
...
//             [self refreshNewStateWithMdel:model];
            if (success.boolValue) {
             //此处需要临时修改cell 上面的为已评论
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    //验证码输入错误
                    UIAlertController *v = [UIAlertController alertControllerWithTitle:@"错误提示" message:errorStr preferredStyle:UIAlertControllerStyleAlert];
                    UIAlertAction *active = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
