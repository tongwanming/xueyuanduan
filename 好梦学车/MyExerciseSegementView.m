...
@implementation MyExerciseSegementView
...
- (instancetype) initWithFrame:(CGRect)frame titleArray : (NSArray *)titleArray block : (btnClickedBlock) clickedBlock
{
    ...
    if (self) {
        ...
        for (int i = 0; i < titleCount; i++) {
            UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(i * SEGEMENT_BTN_WIDTH*TYPERATION, 0, SEGEMENT_BTN_WIDTH*TYPERATION, 48)];
            ...
            btn.titleLabel.font = [UIFont systemFontOfSize:16*TYPERATION];
            [self.segementScrollView addSubview:btn];
            [btnArray addObject:btn];
        }
        self.selectedLine.frame = CGRectMake(10, 38+5, SEGEMENT_BTN_WIDTH*TYPERATION-20, 1.5);
        [self.segementScrollView addSubview: self.selectedLine];
        self.segementScrollView.contentSize = CGSizeMake(titleCount * SEGEMENT_BTN_WIDTH*TYPERATION, 0);
    }
    return self;
}
...
- (void)refreshSegement
{
    ...
    if (currentBtn.frame.origin.x + SEGEMENT_BTN_WIDTH*TYPERATION > self.frame.size.width + self.segementScrollView.contentOffset.x) {
        [self.segementScrollView setContentOffset:CGPointMake(self.segementScrollView.contentOffset.x + SEGEMENT_BTN_WIDTH*TYPERATION, 0) animated:YES];
    }
    ...
    [UIView animateWithDuration:0.2 animations:^{
        _selectedLine.frame = CGRectMake(currentBtn.frame.origin.x+10, self.frame.size.height - 7, SEGEMENT_BTN_WIDTH*TYPERATION-20, 2);
