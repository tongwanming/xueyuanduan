...
@interface SubjectForthViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,SubjectOneTopViewDelegate>

@property (nonatomic, strong) UICollectionView *collectView;

@end

@implementation SubjectForthViewController{
...
- (NSString *)getMyMistake{
    NSString *str;
    NSArray *arr = [[SqliteDateManager sharedManager] getAllDataWithType:@"myMistakef"];
...
- (NSString *)getCollection{
    NSString *str;
    NSArray *arr = [[SqliteDateManager sharedManager] getAllDataWithType:@"collectionf"];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    if ([self.delegate respondsToSelector:@selector(subjectForthViewControllerTapWithIndex:)]) {
        [self.delegate performSelector:@selector(subjectForthViewControllerTapWithIndex:) withObject:@(indexPath.row)];
    }
}
...
- (void)subjectOneTopViewBtnClickWithBtn:(id)sender{
    ...
    if ([self.delegate respondsToSelector:@selector(subjectForthViewControllerTapWithIndex:)]) {
        [self.delegate performSelector:@selector(subjectForthViewControllerTapWithIndex:) withObject:@(btn.tag)];
    }
}
