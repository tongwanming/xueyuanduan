...
@implementation MyLearnScrollView
...
- (instancetype) initWithFrame:(CGRect)frame titleArray:(NSArray *)titleArray viewArray:(NSArray *)viewArray
{
    ...
    if (_mySegementView == nil) {
        _mySegementView = [[MyLearningSegementView alloc] initWithFrame:CGRectMake(0, 0, SCROLLVIEW_WIDTH, SEGEMENT_HEIGTHT) titleArray:titleArray block:^void(int index){  //用block实现回调，顶部按钮点击的时候滚动到指定位置
            [_myScrollView setContentOffset:CGPointMake((index - 1) * SCROLLVIEW_WIDTH, 0)];
            if ([self.delegate respondsToSelector:@selector(myLearnScrollViewBtnClickWithIndex:)]) {
                [self.delegate performSelector:@selector(myLearnScrollViewBtnClickWithIndex:) withObject:@(index)];
            }
        }];
    }
