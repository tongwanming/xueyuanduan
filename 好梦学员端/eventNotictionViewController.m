...
- (void)setModel:(SettingPersonNewsModel *)model{
    ...
    if (model.imageUrl) {
        [_showImageView sd_setImageWithURL:[NSURL URLWithString:_model.imageUrl] placeholderImage:[UIImage imageNamed:@"pic04"]];
    }else{
        _showImageView.image = [UIImage imageNamed:@"pic04"];
    }
...
- (void)getData{
    ...
    if (userId && userId.length >0) {
        dic =@{@"msgType":@"PROMOTION",@"platform":@"",@"pushClient":@"STUDENT",@"pushStatus":@"",@"tags":userId};//0->未开始学习，1->学习中，2->暂停学习，3->申请考试，4->考试通过，5->补考中, 6->考爆
    }else{
