...
- (void)searchRouteWithFirstLocationModel:(FirstLocationModel *)model{
    ...
    //    start.name = @"龙头寺";
    start.pt = (CLLocationCoordinate2D){_userLocationState.location.coordinate.latitude,_userLocationState.location.coordinate.longitude};
...
- (BMKAnnotationView *)mapView:(BMKMapView *)mapView viewForAnnotation:(id<BMKAnnotation>)annotation {
    ...
    if ([annotation isKindOfClass:[BMKPointAnnotation class]]) {
        ...
        //设置图片
//        newAnnotation.image = [UIImage imageNamed:@"seaKing"];
